...
class DQNAgent:
    def __init__(self):
        # CartPole-v1 状态是4维：[小车位置, 小车速度, 杆角度, 杆角速度]
        self.state_dim     = 4
        # CartPole-v1 动作是2维：0(左移)和1(右移)
        self.action_dim    = 2  
        self.lr            = 0.01
        self.gamma         = 0.98
        self.tau           = 0.01
    ...
    def choose_action(self, state):
        ...
        if self.epsilon < random_number:
            with torch.no_grad():
                # 选择Q值最大的动作
                action = int(torch.argmax(self.Q(state)).numpy())
                real_action = action  # CartPole直接使用离散动作
                maxQ_action_count = 1
        else:
            # 随机选择动作（0或1）
...
def generate_cartpole_training_data(
    DQN: DQNAgent,
    n_deriv: int = 2,
    dt: float = 0.02,  # CartPole默认步长
    K_steps: int = 10,
    ...
    """生成CartPole训练数据（适配4维状态+1维离散动作）"""
    env_name = "CartPole-v1"
    n = 4  # 状态维度：[小车位置, 小车速度, 杆角度, 杆角速度]
    m = 1  # 动作维度：离散动作
    ...
    for ep in trange(num_episodes):
        ...
        while not done:
            state_prev_tensor = torch.tensor(state_prev, dtype=torch.float32).unsqueeze(0)
            u_curr = DQN.choose_action(state_prev_tensor)[1]
            state_next, _, done, _ = env.step(u_curr)  # CartPole不需要将动作包装成列表
        ...
        if max_start_idx >= 0:
            for start_idx in range(0, max_start_idx + 1, 1):
...
def generate_cartpole_test_data(
    DQN: DQNAgent,
    K_steps: int = 10,
    num_episodes: int = 100,
    max_steps: int = 500,  # CartPole-v1最大步数是500
    save_path: str = "./data/cartpole",
    seed: int = 2,
...
if __name__ == '__main__':
    ###### logging ######
    log_name = '0404_cartpole'
    ###### logging ######

    agent = DQNAgent()
    model_path = "./data/cartpole/DQN_Q_network.pt"
    ...
    if os.path.exists(model_path):
        ...
        for EP in range(EPISODE):
            ...
            while not done:
                ...
                state_prime, reward, done, _ = env.step(real_action)  # CartPole使用离散动作
            ...
            print(f"EP:{EP}, Score:{score:.1f}, MaxQ_Action_Count:{maxQ_action_count}, Epsilon:{agent.epsilon:.5f}")
        ...
        # 绘制得分曲线
        plt.plot(score_list)
        plt.title('CartPole-v1 Training Scores')
        plt.xlabel('Episode')
        plt.ylabel('Score')
        plt.savefig('./data/cartpole/training_scores.png')
        plt.show()
    ...
    # 生成训练数据
    generate_cartpole_training_data(
        DQN=agent,
        n_deriv=2,
        K_steps=15,
        num_episodes=500,
        save_path="./data/cartpole"
    )
    
    # 生成测试数据
    generate_cartpole_test_data(
        DQN=agent,
        K_steps=15,
        num_episodes=100,
